...
object InviteHandler {
    ...
    class InvitePlayer : IMessageHandler<InvitePlayerMessage.Request, IMessage> {
        ...
        override fun onMessage(message: InvitePlayerMessage.Request, ctx: MessageContext): IMessage? {
            ...
            val playerToInvite = player.server!!.playerList.getPlayerByUsername(message.playerToInvite)
            ...
            if (playerToInvite == null) {
                ...
                ServerAPIHelper.getPlayer(player.uniqueID, listOf("civilization"), Consumer { response ->
                    if (response.civilization == null) {
                        CommonProxy.NETWORK_WRAPPER.sendTo(InvitePlayerMessage.Response(ChatColor.RED + "You are not part of a civilization"), player as EntityPlayerMP)
                    } else if (playerToInvite.uniqueID == player.uniqueID) {
                        CommonProxy.NETWORK_WRAPPER.sendTo(InvitePlayerMessage.Response(ChatColor.RED + "You cannot invite yourself"), player as EntityPlayerMP)
                    } else {
                        ...
                        CommonProxy.NETWORK_WRAPPER.sendTo(InviteRequestMessage.Request(player.name + " is inviting you to join the civilization of " + response.civilization!!.primaryColor + response.civilization!!.name), playerToInvite)
                        CommonProxy.NETWORK_WRAPPER.sendTo(InvitePlayerMessage.Response(ChatColor.BLACK + "Invite sent"), player as EntityPlayerMP)
                    }
                }, Consumer { error ->
                    CommonProxy.NETWORK_WRAPPER.sendTo(InvitePlayerMessage.Response(ChatColor.RED + "An error: " + error.message), player as EntityPlayerMP)
                })
            }
    ...
    class ResponseHandler : IMessageHandler<InviteRequestMessage.Response, IMessage> {
        override fun onMessage(message: InviteRequestMessage.Response, ctx: MessageContext): IMessage? {
            ...
            if (playerModel == null) {
                ...
                ServerAPIHelper.addPlayerToCivilization(player.uniqueID, playerModel.civilization!!.id, Consumer {response ->
