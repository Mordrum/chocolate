...
import com.mordrum.mcore.common.PartiallyMinedBlock;
...
public class MultiMineClient {
    ...
    MultiMineClient() {
        ...
        RenderGlobalExtension renderGlobal = (RenderGlobalExtension) mc.renderGlobal;
        vanillaDestroyBlockProgressMap = renderGlobal.getDamagedBlocks();
    }
    ...
    /**
     * Called by the transformed PlayerControllerMPMixin, has the ability to override the internal block mine completion with another value
     *
     ...
     * @return value to override blockCompletion in PlayerControllerMPMixin with
     */
    ...
    public float eventPlayerDamageBlock(BlockPos pos, float blockCompletion) {
        ...
        for (PartiallyMinedBlock aPartiallyMinedBlocksArray : partiallyMinedBlocksArray) {
            if (aPartiallyMinedBlocksArray != null && aPartiallyMinedBlocksArray.getPos().equals(pos)) {
                float savedProgress = aPartiallyMinedBlocksArray.getProgress();
        ...
        if (!cachedProgressWasAhead) {
            if (!curBlock.equals(pos)) {
                ...
                MCore.instance().getNetworkHelper().sendPacketToServer(new PartialBlockPacket(thePlayer.getName(), curBlock.getX(), curBlock.getY(), curBlock.getZ(), blockCompletion));
                lastBlockCompletion = blockCompletion;
