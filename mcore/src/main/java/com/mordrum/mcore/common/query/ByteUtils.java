...
class ByteUtils {
	/**
	 * Creates and returns a new array with the values of the original from index <code>a</code> to index <code>b</code> and of size
	 * <code>(b-a)</code>.
	 *
	...
	/**
	 * Functions similarly to the standard java <code>String.trim()</code> method (except that null bytes (0x00), instead of whitespace, are stripped
	 * from the beginning and end). If the input array alread has no leading/trailing null bytes, is returned unmodified.
	 *
	...
	/**
	 * Creates an new array of length <code>arr+amount</code>, identical to the original, <code>arr</code>, except with <code>amount</code> null bytes
