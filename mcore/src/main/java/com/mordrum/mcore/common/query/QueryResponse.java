...
public class QueryResponse {
	...
	public QueryResponse(byte[] data, boolean fullstat) {
		...
		if (!fullstat) {
			motd = new String(ByteUtils.subarray(temp[0], 1, temp[0].length - 1), Charset.forName("UTF-8"));
			gameMode = new String(temp[1], Charset.forName("UTF-8"));
			mapName = new String(temp[2], Charset.forName("UTF-8"));
			onlinePlayers = Integer.parseInt(new String(temp[3], Charset.forName("UTF-8")));
			maxPlayers = Integer.parseInt(new String(temp[4], Charset.forName("UTF-8")));
			port = ByteUtils.bytesToShort(temp[5]);
			hostname = new String(ByteUtils.subarray(temp[5], 2, temp[5].length - 1), Charset.forName("UTF-8"));
		} else //full stat
		{
			motd = new String(temp[3], Charset.forName("UTF-8"));
			gameMode = new String(temp[5], Charset.forName("UTF-8"));
			mapName = new String(temp[13], Charset.forName("UTF-8"));
			onlinePlayers = Integer.parseInt(new String(temp[15], Charset.forName("UTF-8")));
			maxPlayers = Integer.parseInt(new String(temp[17], Charset.forName("UTF-8")));
			port = Short.parseShort(new String(temp[19], Charset.forName("UTF-8")));
			hostname = new String(temp[21], Charset.forName("UTF-8"));

			//only available with full stat:
			gameID = new String(temp[7], Charset.forName("UTF-8"));
			version = new String(temp[9], Charset.forName("UTF-8"));

			String pluginsString = new String(temp[11], Charset.forName("UTF-8"));
			int colonCharPlace = pluginsString.indexOf(':');

			serverSoftware = pluginsString.substring(0, colonCharPlace);

			String[] pluginArray = pluginsString.substring(colonCharPlace + 2).split("; ");
			pluginList = Arrays.asList(pluginArray);


			playerList = new ArrayList<>();
			for (int i = 25; i < temp.length; i++) {
				playerList.add(new String(temp[i], Charset.forName("UTF-8")));
			}
		}
	}
	...
	public String asJSON() {
		...
		if (fullstat) {
			...
			json.append("\"serverSoftware\":");                    // "serverSoftware":
			json.append('"').append(serverSoftware).append("\",");// "CraftBukkit on Bukkit 1.4.5-R0.2",

			json.append("\"plugins\":");
			json.append('[');
			for (String plugin : pluginList) {
				json.append("\"").append(plugin).append("\"");
			...
			for (String player : playerList) {
				json.append("\"").append(player).append("\"");
	...
	public String toString() {
		...
		if (fullstat) {
			...
			str.append(version);
			str.append(delimiter);
			str.append(serverSoftware);

			//plugins for non-vanilla (eg. Bukkit) servers
			if (pluginList.size() > 0) {
				str.append(delimiter);
				str.append("Plugins: ");
				str.append(pluginList);
			}
