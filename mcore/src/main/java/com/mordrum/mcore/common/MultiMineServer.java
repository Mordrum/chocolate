...
public class MultiMineServer {
    ...
    private final long initialBlockRegenDelay = 1000L;
    ...
    MultiMineServer() {
        ...
        instance = this;
        blockRegenQueue = new BlockRegenQueue(2048, new BlockAgeComparator());
    ...
    public void onClientSentPartialBlockPacket(EntityPlayerMP player, int x, int y, int z, float value) {
        ...
        newblock.setLastTimeMined(System.currentTimeMillis() + this.initialBlockRegenDelay);
        for (PartiallyMinedBlock iterBlock : partiallyMinedBlocks) {
            if (iterBlock.equals(newblock)) {
                ...
                iterBlock.setLastTimeMined(System.currentTimeMillis() + this.initialBlockRegenDelay);
    ...
    private void sendPartiallyMinedBlockDeleteCommandToAllPlayers(PartiallyMinedBlock block) {
        MCore.instance().getNetworkHelper().sendPacketToAllAroundPoint(new PartialBlockRemovalPacket(block.getPos()),
    ...
    private void sendPartiallyMinedBlockUpdateToAllPlayers(PartiallyMinedBlock block) {
        MCore.instance().getNetworkHelper().sendPacketToAllAroundPoint(new PartialBlockPacket("server", block.getPos().getX(), block.getPos().getY(), block.getPos().getZ(), block.getProgress()),
    ...
    private void sendPartiallyMinedBlockToPlayer(EntityPlayerMP p, PartiallyMinedBlock block) {
        MCore.instance().getNetworkHelper().sendPacketToPlayer(new PartialBlockPacket("server", block.getPos().getX(), block.getPos().getY(), block.getPos().getZ(), block.getProgress()), p);
    }
    ...
    public void onTick(TickEvent.WorldTickEvent tick) {
        if (tick.phase == Phase.END) {
            ...
            if (blockRegenQueue.peek().getLastTimeMined() + this.initialBlockRegenDelay < curTime) {
